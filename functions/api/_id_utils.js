export async function generateNextId(db, table, format, stationCode) {
    ...
    // 3. Query DB for max ID with this prefix
    // For D1, we use LIKE for prefix matching
    const sql = `SELECT id FROM ${table} WHERE id LIKE ? ORDER BY id DESC LIMIT 1`;
    const { results } = await db.prepare(sql).bind(prefix + '%').all();
